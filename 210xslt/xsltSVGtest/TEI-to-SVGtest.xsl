...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="xml-tree" as="document-node()" select="doc('Chapter11-12copy.xml')"/>
    ...
    <xsl:template match="/">
        <svg viewBox="0 0 600 1000">
            ...
            <desc>SVG created from <xsl:apply-templates select="//Chapter11-12copy.xml"/></desc>
           ...
           <g transform="translate(20 900)">
               ...
               <g class="Reference String">
                   <xsl:variable name="Swearing" as="attribute()+" 
                       select="descendant::rs/@swear"/>
                   
                  <xsl:variable name="sortedSwears" as="xs:string+" 
                       select="$Swearing => distinct-values()"/>
                  ...
                  <xsl:variable name="countSwears" as="xs:string+">
                      <xsl:for-each select="$Swearing">
                          <xsl:sort select="$xml-tree//rs[@swear=current()]//swear => count()" 
                              order="descending"/>
